...
@onready var hp_row: HBoxContainer = $Hud/HpRow
@onready var hp_label: Label = $Hud/HpRow/HpLabel
@onready var health_bar = $Hud/HpRow/HealthBar
@onready var mp_row: HBoxContainer = $Hud/MpRow
@onready var mp_label: Label = $Hud/MpRow/MpLabel
@onready var mana_bar = $Hud/MpRow/ManaBar
@onready var status_container: HBoxContainer = $Hud/StatusRow
...
func _style_labels() -> void:
	...
	for lbl in [hp_label, mp_label, intent_label]:
		if lbl == null:
			continue
		if ResourceLoader.exists(serif):
	...
	if intent_label:
		if ResourceLoader.exists(serif):
			intent_label.add_theme_font_override("font", load(serif))
		intent_label.add_theme_font_size_override("font_size", UITypography.FONT_SIZE_CAPTION)

...
func _apply_portrait_theme():
	...
	if not theme:
		...
		theme = preload("res://resources/ui_theme.tres")
...
func _setup_status_container():
	...
	if status_container:
		...
		status_container.position = Vector2(PORTRAIT_SIZE.x - (MAX_STATUS_ICONS * (STATUS_ICON_SIZE + 2)) - 4, PORTRAIT_SIZE.y - STATUS_ICON_SIZE - 4)
...
func get_health_status_text() -> String:
	...
	if health_percentage >= 75.0:
		return "Healthy"
	elif health_percentage >= 50.0:
		return "Injured"
	elif health_percentage >= 25.0:
